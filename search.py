...
"""
Поиск осущестляется по следующей схеме: 
1) Выгружаются все регионы, ищется наилучшее совпадение
2а) Если регион (верхнего уровня) - город переходим к соответствующему методу поиска
2б) Если это не город (респ., обл., и тд.) выгружем районы и города региона и ищем совпаденеи как на первом шаге
3) В обоих случаях далее запускается соответствующий метод, в зависимости от того, что ищем дом или квартиру,
но с по разному заполнеными form-data для поиска
4) Если приходит запрос с succes = false, то отправляем его в очередь SEARCH_SHEDULE для повторного поиска 
спустя некоторый промежуток времени

В начале поиска инкрементируется счетчик попыток.
"""
